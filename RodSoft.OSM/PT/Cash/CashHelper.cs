...
";
            ...
            {
                throw new Exception("Ошибка при создании БД маршрутов. " + ex.Message, ex);
            }
        }
...
//                        prevTime = (DateTime)reader["Time"];
        ...
        public OSMPTStopPoint ReadPoint(Int64 id, bool isWay)
        {
            OSMPTStopPoint point = null;
            using (SQLiteCommand command = new SQLiteCommand(_Connection))
            {
        ...
        private static OSMPTStopPoint LoadPointValues(OSMPTStopPoint point, SQLiteDataReader reader)
        {
            if(point == null)
                point = new OSMPTStopPoint();
...
//            point.IsWay = (byte)reader["IsWay"] == 1;
        ...
        public OSMPTRoute ReadRoute(Int64 id)
        {
            OSMPTRoute route = null;
            using (SQLiteCommand command = new SQLiteCommand(_Connection))
            {
...
//                                        route.To = reader["to"] is DBNull ? "" : (string)reader["to"];
                ...
                if (route != null)
                {
                    ...
                    while (reader.Read())
                    {
                        OSMPTStopPoint point = ReadPoint((Int64)reader["NodeId"], (Convert.ToInt16(reader["type"]) & 64) != 0);
                        	if(point != null)
                        	{
                        	OSMPTRouteStop stop = new OSMPTRouteStop();
        ...
        public IList<OSMPTStopPoint> ReadStops(RectLatLng viewRect)
        {
            IList<OSMPTStopPoint> stopPoints = new List<OSMPTStopPoint>();
            using (SQLiteCommand command = new SQLiteCommand(_Connection))
            {
                ...
                while (reader.Read())
                {
                    OSMPTStopPoint stop = LoadPointValues(null, reader);
        ...
        public IList<OSMPTRouteLite> SelectRoutesForStop(Int64 stopId, IList<OSMPTRouteLite> routes)
        {
            if(routes == null)
                routes = new List<OSMPTRouteLite>();
            using (SQLiteCommand command = new SQLiteCommand(_Connection))
            {
                ...
                while (reader.Read())
                {
                	...
                	{
                    OSMPTRouteLite route = new OSMPTRouteLite();
