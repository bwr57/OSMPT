...
namespace OsmSharp.Routing.Graph.Router.Dykstra
{
    ...
    public class DykstraRoutingLive : DykstraRoutingBase<SimpleWeighedEdge>, IBasicRouter<SimpleWeighedEdge>
    {
        ...
        /// 
        /// </summary>
        public Dictionary<PathSegment<long>, string> Points = new Dictionary<PathSegment<long>, string>();
        /// <summary>
...
//                                log.Add(neighbour_route.ToString() + " " + neighbour_route.Weight.ToString());
                            }
                        }
                    }
                }
                ...
                if (heap.Count > 0)
                {
                    ...
                    while (current != null &&
                        chosen_vertices.Contains(current.VertexId))
                    { // keep dequeuing.
                        log.Add("Повтор " + current.VertexId.ToString() + "  " + current.ToString());
                        current = heap.Pop();
                    }
                    if (current != null)
                    {
                        chosen_vertices.Add(current.VertexId);
                        //log.Add("Обрабатывается " + current.VertexId.ToString() + " : " + current.ToString());
                        //Points.Add(current, current.Weight.ToString() + " : " + current.VertexId.ToString() + " : " + current.ToString());
                    }
                }
